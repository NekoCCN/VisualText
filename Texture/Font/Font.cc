...
using namespace vttexture;

Font::Font(SDL_IOStream* iostream, float ptsize, bool closeio)
{
...
Font::Font(vtasset::BinaryPack BP, uint32_t index, float ptsize)
{
	const char* buffer = BP[index];
...
SDL_Surface* Font::getTextSurface_Blended(const char* text, size_t length, SDL_Color foreground, int32_t wrap_length)
{
	if (wrap_length != 0)
	{
...
SDL_Surface* Font::getTextSurface_LCD(const char* text, size_t length, SDL_Color foreground, SDL_Color background, int32_t wrap_length)
{
	if (wrap_length != 0)
	{
...
SDL_Surface* Font::getTextSurface_Shaded(const char* text, size_t length, SDL_Color foreground, SDL_Color background, int32_t wrap_length)
{
	if (wrap_length != 0)
	{
